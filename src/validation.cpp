...
#if defined(NDEBUG)
# error "Arowanacoin cannot be compiled without assertions."
#endif
...
{
    CAmount nSubsidy = 1800 * COIN;    
    int times = nHeight / consensusParams.nSubsidyHalvingInterval;
    if (nHeight <= 40)
    {
        nSubsidy = 7040000 * COIN;
    }
    else if (nHeight >40 && nHeight < consensusParams.nSubsidyHalvingInterval)
    {
        nSubsidy = 1 * nSubsidy;
    }
    else if( nHeight >= consensusParams.nSubsidyHalvingInterval && nHeight < 2*consensusParams.nSubsidyHalvingInterval)
    {
        nSubsidy = 0.7 * nSubsidy;
    }
    else
    {
       nSubsidy = 0.7 * nSubsidy;
