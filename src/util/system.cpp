...
const char * const BITCOIN_CONF_FILENAME = "arowanacoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "arowanacoind -foo=bar
...
#else
    const char* pszModule = "arowanacoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Arowanacoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Arowanacoin
    // Mac: ~/Library/Application Support/Arowanacoin
    // Unix: ~/.arowanacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Arowanacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Arowanacoin";
#else
    // Unix
    return pathRet / ".arowanacoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Arowanacoin Core copyright is not removed by accident
    if (copyright_devs.find("Arowanacoin Core") == std::string::npos) {
                std::string strYear = strPrefix;
        strYear.replace(strYear.find("2011"), sizeof("2011")-1, "2009");
        strCopyrightHolders += "\n" + strYear + "The Arowanacoin Core developers";
    }
