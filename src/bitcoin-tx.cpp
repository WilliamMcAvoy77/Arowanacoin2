...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " arowanacoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  arowanacoin-tx [options] <hex-tx> [commands]  Update hex-encoded arowanacoin transaction\n" +
            "or:     arowanacoin-tx [options] -create [commands]   Create hex-encoded arowanacoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded arowanacoin transaction
